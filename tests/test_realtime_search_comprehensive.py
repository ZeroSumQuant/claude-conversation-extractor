...
from realtime_search import (
    SearchState, KeyboardHandler, TerminalDisplay, 
    RealTimeSearch, create_smart_searcher
)
from search_conversations import SearchResult
...
class TestKeyboardHandlerComprehensive(unittest.TestCase):
    ...
    def test_unix_keyboard_special_keys(self):
        ...
        with patch('realtime_search.termios'), \
             ...
             patch('realtime_search.select') as mock_select, \
...
class TestTerminalDisplayComprehensive(unittest.TestCase):
    ...
    def _get_stdout_content(self, mock_stdout):
        ...
        return ''.join(call[0][0] for call in mock_stdout.write.call_args_list 
                      if call[0][0])
...
class TestCreateSmartSearcherComprehensive(unittest.TestCase):
    ...
    def test_smart_search_with_semantic(self):
        ...
        def search_side_effect(query, mode=None, **kwargs):
            if mode == "semantic":
                return [Mock(file_path=Path("/semantic/result"), timestamp=datetime.now())]
            return []
